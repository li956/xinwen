<template>
  <div>
    <div class="bg_content">
      <div class="shade"></div>
      <div class="bg_alter_pwd">
        <label class="regist">找回密码</label>
        <div class="input">
          <input
            type="text"
            placeholder="输入手机号"
            v-model="alter_account"
            @blur="check_account"
            autofocus="true"
          >
          <label>{{tip1}}</label>
        ...
        <div class="input">
          <input type="password" placeholder="输入新密码" v-model="regist_pwd" @blur="check_pwd">
          <label>{{tip4}}</label>
        </div>
        <div class="input">
          <input
        ...
        <button class="to_login" @click="to_login">完成</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    //验证手机号
    check_account() {
      if (this.alter_account) {
        var reg = /^1[34578]\d{9}$/;
        if (!reg.test(this.alter_account)) {
          this.tip1 = "手机号码格式不正确!";
          return false;
        } else {
          this.tip1 = "";
    ...
    //获取验证码
    get_captcha_info() {
      if (("reset", this.alter_account && this.captcha_img)) {
        getSmsCaptcha(this.captcha_img, this.alter_account).then(res => {
    ...
    to_login() {
      if (
        ...
        getRegistInfo("reset", this.alter_account).then(res => {
          if (res.data.code === "success") {
            alert("修改成功");
